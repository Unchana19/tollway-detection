...
def create_detection_history_image(detection_history, width=1200, height=800, selected_lane=0):
    ...
    # Column widths - adjusted for larger window
    time_col_width = 280
    type_col_width = 200
    lane_col_width = 100
    id_col_width = 100
    status_col_width = width - time_col_width - type_col_width - lane_col_width - id_col_width - 40
    ...
    # Draw table title with modern styling - larger and positioned higher to avoid overlap
    title_shadow = history_image.copy()
    cv2.putText(title_shadow, "Recent Detection History", 
               (width//2 - 202, 42),
               cv2.FONT_HERSHEY_DUPLEX, 1.5, (220, 220, 220), 2, cv2.LINE_AA)
    cv2.addWeighted(title_shadow, 0.3, history_image, 0.7, 0, history_image)
    cv2.putText(history_image, "Recent Detection History", 
               (width//2 - 200, 40),
               cv2.FONT_HERSHEY_DUPLEX, 1.5, (40, 40, 80), 2, cv2.LINE_AA)
    ...
    col_x = 20
    headers = ["Detection Time", "Vehicle Type", "Lane", "ID", "Payment Status"]
    col_widths = [time_col_width, type_col_width, lane_col_width, id_col_width, status_col_width]
    ...
    for i in range(max_rows):
        ...
        try:
            # Try to parse the time and show "time ago" if possible
            ...
            pass  # Keep original time format if parsing fails
        ...
        tag_width = len(vehicle_type) * 7 + 20
